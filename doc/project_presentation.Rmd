...
apply_filter <- function(data, input){
  data <- data %>% 
    filter(str_detect(str_to_lower(Snake), str_to_lower(input$snake)),
           str_detect(str_to_lower(Note), str_to_lower(input$country)),
           )
}
...
server <- function(input, output) {
  dataset <- reactive({
      data_aug %>%
        ...
        mutate(value = round(value, 2)) %>% 
        rename(Value = value) %>% 
        mutate(Snake = reorder(Snake, desc(Snake)))
    })
  ...
  output$plot <- renderPlotly({
    plot <- dataset() %>% 
      ...
      coord_flip() +
      theme(legend.position = "none")
    if( input$dataset == "Relative Abundance"){
      plot +
        ylab('Venom composition (%)')
    }else{
      plot +
        ylab('Venom composition (mg)')
    }
  })
}
