...
ui <- fluidPage(
  fluidRow(
    column(4,
           wellPanel(
             selectInput("genus", label = "Genus:", choices = data_aug$Genus, multiple = TRUE),
             selectInput("species", label = "Species:", choices = data_aug$Species, multiple = TRUE)
             # textInput("snake2", "Another snake:", value = "")
           )
    ),
    column(8,
...
apply_filter <- function(data, input){
  data <- data %>% 
    filter(if( !is.null(input$genus) ){
        data <- data %>% 
          filter(str_to_lower(Genus) %in% str_to_lower(input$genus))
  },
      if( !is.null(input$species) ){
        data <- data %>% 
          filter(str_to_lower(Species) %in% str_to_lower(input$species))
  }else{data}
           )
}
